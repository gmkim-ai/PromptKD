...
class PPOTrainer():
    ...
    def train(self):
        ...
        self.epoch_temp = self.sampler.epochs
        self.evaluate()
    ...
    def evaluate(self):
        ...
        if get_rank() == 0:
            ...
            if self.best_rougeL < eval_results["rougeL"]:
                save_dir = os.path.join(self.args.save, "best_rougeL")
                os.makedirs(save_dir, exist_ok=True)

                with open(os.path.join(save_dir, "log.txt"), "w") as f:
                    f.write("step: %d, rougeL: %f\n" % (self.global_iter_count, eval_results["rougeL"]))

                if self.args.model_parallel:
                    if get_rank() == 0:
                        self.model.module.config.to_json_file(os.path.join(save_dir, "config.json"))
                        self.tokenizer.save_pretrained(save_dir)
                    # if mpu.get_data_parallel_rank() == 0:
                    #     save_parallel(self.model.module.base_model, save_dir)
                else:
                    if get_rank() == 0:
                        self.model.module.base_model.save_pretrained(save_dir, safe_serialization=False)
                        # torch.save(self.model.module.value_model.state_dict(), os.path.join(ckpt_dir, "value_model.ckpt"))
                        print(f"Model save to {save_dir}")
                        self.tokenizer.save_pretrained(save_dir)
                
                self.best_rougeL = eval_results["rougeL"]
